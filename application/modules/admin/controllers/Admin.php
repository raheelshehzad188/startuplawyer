...
// 		$user = $this->session->userdata('knet_login');
		...
		{   
			$this->template->admin('dash1',$data);
		}
		...
		if(isset($_REQUEST["type"])  && isset($_REQUEST['user_id']) && isset($_REQUEST['order_id']))
{
	...
	if($_REQUEST["type"] == 's')
	{
		...
		$product->updatemeta('user',$uid,'pack_expire_date',$date);
		$product->updatemeta('user',$uid,'ppack',$pid);
		global $wpdb;
    ...
    foreach($myrows as $cat)
    {
        ...
        {
            $cts[] = $cat->term_id;
            $product->updatemeta('user',$uid,$cat->slug,$postno);
        }
    ...
    $ucats = get_user_meta($uid,'scats',true);
                        ...
                        $product->updatemeta('user',$uid,'scats',implode(",",$ucats));
		//change end
...
// 		$data['data']= $products;
	...
	$breed = array();
		...
		$data['js']= $js;
		$data['breed']= $breed;
		$this->template->admin('service_provider',$data);
...
require_once( ABSPATH . 'wp-admin/includes/image.php' );
        ...
        $product->updatemeta('user',$user_id, 'wp_user_avatar', $attach_id);
    }
    public function profile( $user_id = 0)
	{
		...
		$modal = $this->Common_model;
	        		...
	        		if($mediaID)
	        		{
	        		$r = $product->updatemeta('user',$user->ID,'banner',$mediaID);
	        		}
	        	}
	        }
		...
		$modal = $this->Common_model;
			...
			elseif(isset($_REQUEST['bio']))
			{
				...
				if(isset($_POST['whaatsapp']))
				{
				    $uid = $user->ID;
				    $product->updatemeta('user',$uid,'whaatsapp',$_POST['whaatsapp']);
				}
				if(isset($_POST['billing_email']))
				{
				    $uid = $user->ID;
				    $product->updatemeta('user',$uid,'billing_email',$_POST['billing_email']);
				}
...
// 		$this->load->view('includes/cfooter',$data);
	...
	public function page($page = "")
	{
		...
		$tit = array(
		    ...
		    $perm = $page;
		    if($page == 'pay' || $page == 'earning_statement' || $page == 'payment_details')
		    {
		        $perm = 'payment';
		    }
		...
		$css[] = $assets.'/app-assets/css/pages/data-list-view.css';
		$js = array();
		$js[] = $assets.'app-assets/vendors/js/tables/datatable/datatables.min.js';
		$js[] = $assets.'/app-assets/js/scripts/ui/data-list-view.js';
		if($page == 'order')
		{
		    $this->load->model('Common_model');
		...
		$this->Common_model->key = 'ID';
		$modal = $this->Common_model;
		$data['data'] = $orders = $modal->get(array('post_type' => 'shop_order'));
		}
