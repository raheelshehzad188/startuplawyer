...
class Products extends CI_Controller {
	...
	public function save($id = 0)
	{
        ...
        else
        {
            ...
            // get remaining product in category
            $cslug= '';
            if($_POST['catID'])
            {
            $cat = get_term($_POST['catID']);
            $cslug = $cat->slug;
            }
            $old = get_user_meta($user->ID,$cslug,true);
            if(!$old)
            {
                $this->session->set_flashdata('error', "Sorry, you can not add more producuts in that category!");
...
set_post_thumbnail($pid,$attach_id);
	        ...
	        if($pid)
	        	{
	        	    ...
	        	    $ret = update_post_meta($pid, '_price',$_POST['price']);
	        	    $ret = update_post_meta($pid, '_sale_price',$_POST['price']);
	        	    }
                    ...
                    $product->save();
                    $new = $old = $old - 1;
                    update_user_meta($user->ID,$cslug,$new);
	        	    }
	...
	public function all()
	{
		...
		foreach ($products as $key => $value) {
			$products[$key]['sale'] = get_post_meta($value['ID'], 'total_sales',true);
		}
