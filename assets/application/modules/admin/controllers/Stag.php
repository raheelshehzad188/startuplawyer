...
class Stag extends CI_Controller {
		...
		$this->url = base_url('/admin/tags');
		$this->load->model('auth_model');
// 		adil_wrok
		$this->load->model('Product_model');
    }
    //variables
    private $single= 'Service Provider Tag';
    private $multi= 'Service Provider Tags';
    private $add= 'addptype';
    private $all= 'allptype';
    private $post_type= 'sp_tag';
    private $url= '';
    private $modal= '';
	/**
        ...
        else
        {
        	...
        	if($id)
        	{
        		...
        		if($pid)
	        	{
	        	    ...
	        	    $this->Product_model->updatemeta('post', $pid,'adv_filter', 1);
	        	    }
	        	    else
	        	    {
	        	     $this->Product_model->updatemeta('post', $pid,'adv_filter', 0);   
	        	    }
        	...
        	else
        	{
	        	...
	        	if($pid)
	        	{
	               ...
	               // adil_work
	                if(isset($_REQUEST['category']) && $_REQUEST['category'])
	        	    {
	        	    $r = $this->Product_model->updatemeta('post', $pid,'adv_filter', 1);
	        	    }
	        	    else
	        	    {
	        	     $this->Product_model->updatemeta('post', $pid,'adv_filter', 0);   
