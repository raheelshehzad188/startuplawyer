...
$('#DatePicker').datepicker({
    ...
    beforeShowDay: function(date) {
        console.log(date);
        var sdate = $('#datepicker_field_new').val();
    var mdate = new Date(sdate);
    console.log(mdate);
    if(mdate == date)
    {
    }
...
$(document).ready(function(){
    ...
    {
        var date = new Date($('#datepicker_field_new').val());
        var DAY = date.getDate()+1;
        $('#DatePicker td').each(function(i, obj) {
            $(this).children().removeClass('ui-state-active');
    if(DAY == $(this).children().text())
    {
        $(this).children().addClass('ui-state-active');
        load_slots($('#datepicker_field_new').val());
    }
});
